import { AuthCodeClass, useCreateAuthCodeMutation, useGetAuthCodesQuery, useRemoveAuthCodeMutation } from "src/generated/graphql"
...
const AuthCodes: React.FC<AuthCodesProps> = () => {
	...
	const [{fetching: AuthCodesCreateFetching}, createAuthCode] = useCreateAuthCodeMutation()
	const [{fetching: AuthCodesRemovingFetching}, removeAuthCode] = useRemoveAuthCodeMutation()
	...
	if (AuthCodesData.getAuthCodes.errors) return (
		<div>
			<p className={styles.p}>An error has occured: {AuthCodesData.getAuthCodes.errors[0].error}</p>
		</div>
	)
	...
	return (
		<div>
			<Grid container spacing={2} direction="row" alignItems="center" justify="center">
				{codes.map((code, i) => <AuthCode code={code} removeAuthCode={removeCode} key={i} />)}
			</Grid>
			{createCode}
		</div>
	)
