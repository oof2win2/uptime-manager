...
const ManagerService: React.FC<ManagerServiceProp> = ({
	service,
	deleteService,
	modifyService,
}) => {
	...
	const editDialog = (
		<Dialog
			open={editDialogEnabled}
			onClose={() => setEditDialogEnabled(false)}
		>
		...
		<Grid item xs="auto">
			...
			>
				...
				<p
					...
					style={{ alignSelf: "center", marginLeft: 8 }}
				>
					Socket type: {service.socketType === "udp" ? "UDP" : "TCP"}
				</p>
				<p
					className={styles.p}
					style={{ alignSelf: "center", marginLeft: 8 }}
				...
				<p
					...
					style={{ alignSelf: "center", marginLeft: 8 }}
				>
					POST updating: {service.postUpdating ? "Yes" : "No"}
				</p>
				<IconButton
				...
				<IconButton
					onClick={() => setEditDialogEnabled(true)}
					style={{ alignSelf: "center" }}
				>
					<EditIcon />
				</IconButton>
